...
本地登录（前台 `/user/login/`、后台 `/company/login/`）默认会**同步登录云端**（cloudLogin / cloudCompanyLogin），
用于给未本地化的代理接口提供云端凭证。云端的会话机制是**单点登录**——同一账号新登录会作废旧会话，
因此**本地登录后，云端原版（同账号）会被踢下线**。这是混合认证的固有行为，与访问哪个页面无关
（登录动作本身即触发，不是某个页面导致）。

如果希望**本地与云端原版同时在线互不干扰**，以 `LZ_NO_CLOUD_SYNC=1` 启动即可：

```bat
set LZ_NO_CLOUD_SYNC=1 && node server.js
```

**取舍**：该模式下登录不创建云端会话（云端原版不受影响），已本地化的 625 个接口完全正常；
未本地化的接口（NIM 聊天、商品单位/厂家等动态数据）失去实时云端认证，auto 模式走录制回放兜底，
offline 模式无影响。需要完整云端代理功能时去掉该变量即可（默认行为）。

**登录稳定性**：`LZ_NO_CLOUD_SYNC=1` 模式下，未本地化接口代理到云端必然返回 10012/10305（无会话），
若透传给前端，axios 拦截器会清除登录凭证并弹回登录页（表现为"不能登录"）。已修复：代理层在该模式下
将 10012/10305 响应宽容改写为 `code:0` 空成功数据，登录稳定、前后台均正常进入主界面；
代价是这些接口返回空数据（如主界面项目计数为 0），不影响已本地化功能。
